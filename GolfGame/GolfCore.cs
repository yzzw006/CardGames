    ...
    class GolfCore
    {
        ...
        {    
            ...
            for (int i = 0; i < _stackCount; i++)
            {
                ...
                _cardStack[i].AddRange(_cardList.GetRange(0, 5));
                _cardList.RemoveRange(0, 5);
            }
        ...
        bool CanMove(int src, int des)
        {
            ...
            if ( src>6 || des != 7)
                return false;
            //只要差为1就能移动
            if (Math.Abs(_homeList.Last().Rank - _cardStack[src].Last().Rank) == 1)
                return true;
            else
                return false;
        }
